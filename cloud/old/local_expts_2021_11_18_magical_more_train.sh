...
# What this file does: train MAGICAL with GAIL (both augs and no-augs) for
# 1,000,000 environment time steps.
...
magical_base_cfgs=("cfg_base_5seed_1cpu_pt25gpu" "tune_run_kwargs.num_samples=5"
                   ...
                   "il_train.gail.total_timesteps=1000000")
...
for magical_env in "${magical_envs[@]}"; do
    for use_augs in augs noaugs; do
        submit_expt "${magical_base_cfgs[@]}" cfg_repl_none \
            ...
            exp_ident="control_gail_nov18_1e6_${use_augs}"
    done
done
wait
