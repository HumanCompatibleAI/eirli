                ...
                if "terminal_observation" in infos[i]:
                    ...
                    if self.channels_first:
                        new_terminal = np.concatenate(
                            ...
                            # FIXME(sam): upstream this is
                            # self.stack_dimension, but that doesn't work when
                            # we are chans-first: indexing into
                            # self.stackedobs[i] discards the leading (batch)
                            # dimension, so we actually need to shift the axis
                            # down by 1 (which is what repeat_axis does).
