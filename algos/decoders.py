...
class ProjectionHead(LossDecoder):
    def __init__(self, representation_dim, projection_shape):
        super(ProjectionHead, self).__init__(representation_dim, projection_shape)

        self.g1 = nn.Linear(self.representation_dim, 256)
        self.g2 = nn.Linear(256, 256)
        self.g3 = nn.Linear(256, self.projection_dim, bias=False)
        self.relu = nn.ReLU()
...
class LSTMHead(LossDecoder):
    def __init__(self, representation_dim, projection_shape):
        super(LSTMHead, self).__init__(representation_dim, projection_shape)
...
class VAEDecoder(LossDecoder):
    ...
    def __init__(self, representation_dim, projection_shape):
        super(VAEDecoder, self).__init__(representation_dim, projection_shape)
        self.relu = nn.ReLU()
        self.dense1 = nn.Linear(self.representation_dim, 64)
        self.dense2 = nn.Linear(64, 64)
        self.deconv1

    def forward(self, z, extra_context=None):
        batch_size = z.shape[0]
...
class PolicyHead(nn.Module):
    ...
    def __init__(self, action_size, representation_dim):
