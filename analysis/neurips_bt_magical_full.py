...
Like `neurips_bt_magical.py`, but prints out full results for all variants.
...
COL_ORDER = [
    ...
    # controls for pretraining + GAIL runs
    'neurips_control_gail_augs',
...
def process_df(df):
    ...
    pivoted = pd.pivot_table(
        df, columns='exp_ident', index=['train_env', 'test_env'],
        values='contents', aggfunc=unique_agg) \
        .sort_index(key=index_sort_key)
