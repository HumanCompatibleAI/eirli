"""
Pair Constructors turn a basic trajectory dataset into a dataset of `context`,
`target`, and `extra_context` data elements, along with a metadata tensor
containing the trajectory ID and timestep ID for each element in the dataset.
        ...
        while i < len(dones) - self.k:
            ...
            if self.mode is None:
                ...
                dataset.append({
                    'context': obs[i],
                    'target': obs[i + self.k],
                    'extra_context': actions[i:i + self.k],
                    'traj_ts_ids': [trajectory_ind, timestep]
                })
            elif self.mode == 'inverse_dynamics':
                dataset.append({
                    'context': obs[i],
                    'target': actions[i:i + self.k],
                    'extra_context': obs[i + self.k],
                    'traj_ts_ids': [trajectory_ind, timestep]
                })
