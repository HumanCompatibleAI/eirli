...
class RepresentationLoss(ABC):
    ...
    def get_vector_forms(self, decoded_context_dist, target_dist, encoded_context_dist):
        decoded_contexts = decoded_context_dist.sample(
        ) if self.sample else decoded_context_dist.loc
        targets = target_dist.sample() if self.sample else target_dist.loc
        encoded_contexts = encoded_context_dist.sample(
        ) if self.sample else encoded_context_dist.loc
...
class AsymmetricContrastiveLoss(RepresentationLoss):
    ...
    def __init__(self, device, sample=False, temp=0.1, normalize=True):
        ...
        # Most methods use either cosine similarity or matrix multiplication
        # similarity. Since cosine similarity equals taking MatMul on
        # normalized vectors, setting normalize=True is equivalent to using
        # torch.CosineSimilarity().
        self.normalize = normalize

        # Sometimes the calculated vectors may contain an image's similarity
        # with itself, which can be a large number. Since we mostly care about
        # maximizing an image's similarity with its augmented version, we
        # subtract a large number to make the classification have ~0
        # probability picking the original image itself.
        self.large_num = 1e9
    ...
    def __call__(self, decoded_context_dist, target_dist, encoded_context_dist=None):
        ...
        decoded_contexts, targets, _ = self.get_vector_forms(decoded_context_dist, target_dist,
...
class QueueAsymmetricContrastiveLoss(AsymmetricContrastiveLoss):
    """
    This implements algorithms that use a queue to maintain all the negative
    examples. The contrastive loss is calculated through the comparison of an
    image with its augmented version (positive example) and everything else in
    the queue (negative examples). The method used in MoCo.

    Alternatively, for higher sample efficiency, one may use (1) current
    batch's augmented images, (2) current batch's original images, and (3) all
    the images in the queue as negative examples. This is implemented with
    setting use_batch_neg=True.
    """
    ...
    def calculate_logits_and_labels(self, z_i, z_j, mask):
        ...
        # Calculate the dot product similarity of each image with all images in
        # the queue. Return an NxK tensor.
        ...
        if self.use_batch_neg:
            ...
            # The values we want to maximize lie on the i-th index of each row
            # i. i.e. the dot product of represent(image_i) and
            # represent(augmented_image_i).
            labels = torch.arange(batch_size, dtype=torch.long).to(self.device)

        else:
            # torch.einsum provides an elegant way to calculate vector dot
            # products across a batch. Each entry on the Nx1 returned tensor is
            # a dot product of represent(image_i) and
            # represent(augmented_image_i).
...
class BatchAsymmetricContrastiveLoss(AsymmetricContrastiveLoss):
    """
    This applies to algorithms that performs asymmetric contrast with samples
    in the same batch. i.e. Negative examples come from all other images (and
    their augmented versions) in the same batch. Represents InfoNCE used in
    original CPC paper.
    """
    ...
    def calculate_logits_and_labels(self, z_i, z_j, mask):
        ...
        # Similarity of the original images with all other original images in
        # current batch. Return a matrix of NxN.
        logits_aa = torch.matmul(z_i, z_i.T)  # NxN
        ...
        # The values we want to maximize lie on the i-th index of each row i.
        # i.e. the dot product of represent(image_i) and
        # represent(augmented_image_i).
...
class SymmetricContrastiveLoss(RepresentationLoss):
    """
    A contrastive loss that does prediction "in both directions," i.e. that
    calculates logits of IJ similarity against all similarities with J, and
    also all similarities with I, and calculates cross-entropy on both
    """
    def __init__(self, device, sample=False, temp=0.1, normalize=True):
    ...
    def __call__(self, decoded_context_dist, target_dist, encoded_context_dist=None):
        ...
        # Each row now contains an image's similarity with the batch's
        # augmented images & original images. This applies to both original and
        # augmented images (hence "symmetric").
...
class CEBLoss(RepresentationLoss):
    """
    A variational contrastive loss that implements information bottlenecking,
    but in a less conservative form than done by traditional VIB techniques
    """
    ...
    def __call__(self, decoded_context_dist, target_dist, encoded_context_dist=None):
        ...
        # B -> Log proba of each vector in z under the distribution it was
        # sampled from
        log_ezx = decoded_context_dist.log_prob(z)
        # B -> Log proba of each vector in z under the distribution conditioned
        # on its corresponding target
        log_bzy = target_dist.log_prob(z)
        cross_probas_logits = torch.stack(
            [target_dist.log_prob(z[i]) for i in range(z.shape[0])],
            dim=0)  # BxB Log proba of each vector z[i] under _all_ target distributions
        # The return shape of target_dist.log_prob(z[i]) is the probability of
        # z[i] under each distribution in the batch.
        # logits of shape BxB -> Batch categorical, one distribution per element in z over possible
        catgen = torch.distributions.Categorical(logits=cross_probas_logits)
        # targets/y values
        inds = (torch.arange(start=0, end=len(z))).to(self.device)
        # The probability of the kth target under the kth Categorical
        # distribution (probability of true y)
