...
def register_dmc_envs():
    ...
    gym.register('DMC-Ball-In-Cup-Catch-v0',
                 entry_point=entry_point,
                 kwargs=dict(domain_name='ball_in_cup', task_name='catch', frame_skip=8))


@benchmark_ingredient.capture
def load_dataset_dm_control(dm_control_env, dm_control_full_env_names, dm_control_demo_patterns,
                            n_traj):
    ...
    dones_lists = [
        ...
        # terminal True value
        np.array([False] * (len(t.acts) - 1) + [True], dtype='bool') for t in loaded_trajs
    ]
    dataset_dict = {
        'obs': np.concatenate([t.obs[:-1] for t in loaded_trajs], axis=0),
        'acts': np.concatenate([t.acts for t in loaded_trajs], axis=0),
        'next_obs': np.concatenate([t.obs[1:] for t in loaded_trajs], axis=0),
        'infos': np.concatenate([t.infos for t in loaded_trajs], axis=0),
        'rews': np.concatenate([t.rews for t in loaded_trajs], axis=0),
        'dones': np.concatenate(dones_lists, axis=0),
    }
